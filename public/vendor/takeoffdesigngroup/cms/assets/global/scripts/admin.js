...
$(function() {
    ...
    if ($('.nav-tabs').length > 0) {
        ...
        if (lastTab && lastTab !== 'null' && lastTab !== '#' && hash === '')
            $('a[href=' + lastTab.replace(prefix,"") + ']').tab('show');
        else
        {
            if (hash !== '#' && hash !== '')
            {
                ...
                case 500:
                    ...
                    setTimeout(function() {
                        $('.messages').removeClass('hidden')
                            .find('.msg .wrap').text('An unexpected error has occurred.  If this persists, please contact an administrator.');
                    }, 500);
                    break;
...
$(window).load(function() {
    if (typeof overrideLoader == 'undefined')
        stopLoading();
});
...
var FormValidation = {
    ...
    init: function() {
        $.validator.setDefaults({
            ...
            invalidHandler: function(form, validator) {
                ...
                if (errors) {
                    messages.removeClass('hidden')
                                       .find('.text-error > span').html(errors == 1 ? 'was an error' : 'were some errors');
                } else {
            ...
            errorPlacement: function(error, element) {
                ...
                if (element.hasClass('bs-select'))
                    error.insertAfter(container.find('.bootstrap-select'));
                else if (element.hasClass('toggle'))
                    error.insertAfter(container.find('.btn-group'));
                else if (element.hasClass('multi-select'))
                    error.insertAfter(container.find('.ms-container'));
                else if (element.attr('type') === 'file')
                    container.find('.validate').append(error);
                else if (container.has('.input-group-addon').length)
                    error.insertAfter(container.find('.input-group'));
                else
    ...
    displayErrors: function(form, errors) {
        ...
        messages.removeClass('hidden');
        messages.find('.alert-danger').removeClass('hidden').find('.wrap').html('Whoops! There <span>were some errors</span> in your form.  Please review your form below and make corrections where necessary.');
        ...
        $.each(errors, function(key, error) {
            ...
            if (element.hasClass('bs-select'))
                error.insertAfter(field.find('.bootstrap-select'));
            else if (element.is(':radio, :checkbox'))
                field.append(error);
            else if (element.hasClass('toggle'))
                error.insertAfter(element.closest('.btn-group'));
            else if (element.hasClass('multi-select'))
                error.insertAfter(element.next('.ms-container'));
            else if (element.is('.control-label'))
                field.append(error.addClass('text-center'));
            else
                error.insertAfter(element);
        });
    },
...
$( "select#site" ).change(function() {
    ...
    if (id == 'add')
        window.location.href = '/'+domain+'/sites/create';
    else if (id > 0)
        window.location.href = '/'+domain+'/sites/' + id + '/change';
});
...
$('#title').keyup(function() {          // Retrieve formatted URL from form element for display
