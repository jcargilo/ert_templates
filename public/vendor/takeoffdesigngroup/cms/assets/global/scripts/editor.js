...
init = {
    initEverything: function () {
        if (select.sectionEditor.length > 0)
            sectionEditor.init();
    }
};
...
sectionEditor = {
    ...
    init: function() {
        ...
        $('#background_min_height', '#background_max_height').keypress(function(e) { return numericOnly(e); });
        $('#background_position_x').keypress(function(e) { return numericOnly(e); });
        $('#background_position_y').keypress(function(e) { return numericOnly(e); });
    }
}
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.error != undefined)
            {
                ...
                FormValidation.displayErrors(form, response.errors);
            else
            {
                // show update message
                var messages = $('form.page > .messages');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.border_color != '')
                    $('#border_color').minicolors('value', data.border_color);
                ...
                if (data.section_background_color != '')
                    $('#section_background_color').minicolors('value', data.section_background_color);
                if (data.page_background_color != '')
                    $('#page_background_color').minicolors('value', data.page_background_color);
                if (data.background_fixed == 1)
                    $('#background_fixed').bootstrapSwitch('toggleState');
                if (data.background_parallax == 1)
                    $('#background_parallax').bootstrapSwitch('toggleState');
                if (data.background_stretch == 1)
                    $('#background_stretch').bootstrapSwitch('toggleState');
                if (data.background_repeat_x == 1)
                    $('#background_repeat_x').bootstrapSwitch('toggleState');
                if (data.background_repeat_y == 1)
                    $('#background_repeat_y').bootstrapSwitch('toggleState');
                ...
                if (data.overlay == 1)
                    $('#overlay').bootstrapSwitch('toggleState');
                ...
                for (var i = 0; i < data.columns.length; i++) {
                    ...
                    $('#col'+column.column).find('.toggle_buttons > label').removeClass('active').children().removeAttr('checked');
                    $('input[name=classes_'+column.column+']').val(column.classes);

                    if(column.template_id != null) {
                        $('#template_'+column.column).prop('checked', true).parent('label').toggleClass('active', true);
                        $('#template_id_'+column.column).val(column.template_id.toString());
                        option = 'template';
                    }
                    else if(column.slideshow_id != null) {
                        $('#slideshow_'+column.column).prop('checked', true).parent('label').toggleClass('active', true);
                        $('#slideshow_id_'+column.column).val(column.slideshow_id.toString());
                        option = 'slideshow';
                    }
                    else {
                        $('#editor_'+column.column).prop('checked', true).parent('label').toggleClass('active', true);
                    ...
                    if (column.metadata) {
                        ...
                        if (column.metadata.background) {
                            ...
                            if (column.metadata.background.color)
                                $(`#column_bg_color_${i+1}`).minicolors('value', column.metadata.background.color);
                        }
                    }
        ...
        success: function(data) { 
            if (data.length != null && data.length > 0) {
                ...
                for(var i = 0; i < data.length; i++) {
                    $('#sections > tbody:last')
                        .append('<tr id="' + i + '" data-id="' + data[i].id + '"><td>'+ data[i].title +'</td>' +
                                    '<td class="text-center hidden-xs">' + data[i].real_layout + '</td>' +
                                    '<td class="text-right">' +
                ...
                $('#sections tbody > tr').remove();
                $('#sections > tbody')
                    .append('<tr id="norows"><td colspan="3" class="text-info small" align="center"><i>No sections have been added to this page yet.</i></td></tr>');
            }
        },
    });
    App.unblockUI('#page_content');
...
function resetSectionForm() {
    ...
    if ($('#background_fixed').bootstrapSwitch('state'))
        $('#background_fixed').bootstrapSwitch('toggleState');
    if ($('#background_parallax').bootstrapSwitch('state'))
        $('#background_parallax').bootstrapSwitch('toggleState');
    if ($('#background_stretch').bootstrapSwitch('state'))
        $('#background_stretch').bootstrapSwitch('toggleState');
    if ($('#background_repeat_x').bootstrapSwitch('state'))
        $('#background_repeat_x').bootstrapSwitch('toggleState');
    if ($('#background_repeat_y').bootstrapSwitch('state'))
        $('#background_repeat_y').bootstrapSwitch('toggleState');
    $('#background_position_x').val('');
    $('#background_position_y').val('');

    if ($('#overlay').bootstrapSwitch('state'))
        $('#overlay').bootstrapSwitch('toggleState');
    $('#overlay_class').val('');
...
function showContentAreas() {
    ...
    $('#panel-content').find('.nav-tabs').toggleClass('hidden', columns == 1);
    for (i = 0; i < 5; i++) {
        if (i < columns) {
...
function showResponsiveSettings(trigger, type, data) {
    ...
    if (classes) {
        ...
        for (const [attribute, value] of Object.entries(classes)) { // padding, margins, vertical_alignment, full_width
            if (typeof(value) === 'object') {
                for (const [setting, v] of Object.entries(value)) { // top, left, bottom, right
...
function resetGroup(group) {
    [...group].forEach(control => {
        ...
        if (control.is('select')) control.val('');
            ...
            if (control.val() === '')
                control.prop("checked", true).parent().addClass('active');
            else
...
function updateResponsiveControlValues(control, value) {
    if (control.is('select')) {
        ...
        if (match.length > 0) {
            control.filter(`[name="${match.attr('name')}"]`).removeAttr("checked").parent().removeClass('active');
...
function updateResponsiveClasses(e) {
    ...
    if (key !== undefined) {
        if (value === '' && classes[attribute][key])
            delete classes[attribute][key];
        else
            value = {...classes[attribute], [key]: value};
    } else if (value === '')
        delete classes[attribute];
    
    if (value !== '')
        classes = {...classes, [attribute]: value};
...
function initTinyMCE(crossDomain, baseUrl) {
    ...
    tinymce.init({
        ...
        statusbar: false,
        plugins: [
            "lists link hr paste media image"
        ],
        toolbar1: "removeformat | undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | hr | media image",
        paste_as_text: true,
        browser_spellcheck: true,
        forced_root_block: 'p', // Set to false to disable.
        convert_urls : 0, // disables converting urls from absolute to relative
    ...
    tinymce.init({
        ...
        toolbar1: "removeformat | undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link unlink | hr",
        paste_as_text: true,
        browser_spellcheck: true,
        forced_root_block: 'p', // Set to false to disable.
        convert_urls : 0, // disables converting urls from absolute to relative
    ...
    tinymce.init({
        ...
        menubar: false,
        statusbar: false,
        toolbar1: "styleselect | undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | hr",
